...
const getUpperPosition = (coords: Coordinate) => {
    const position = { ...coords };
    position.y -= 1;
    return position;
};

const getRightOrLeftPosition = (coords: Coordinate) => {
    if (coords.x === 0) {
...
const recurse = (runway: Runway, currentPosition: Coordinate, stepCount: number): number => {
    const upperPosition = getUpperPosition(currentPosition);
    ...
    if (isWarrior) {
        const nextPos = getRightOrLeftPosition(currentPosition);
